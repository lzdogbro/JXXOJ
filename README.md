...
- **🎓 作业功能**：课程作业的发布与提交，支持学生组管理、时间窗约束、AC 制完成判定、完成统计与未完成提醒
...
- **🐛 Bug 修复**：修复 Redis 分布式锁问题、PK 对战死锁、Lombok 升级导致评测反序列化失败（提交全部 Failed）等关键 bug
...
导航栏新增「作业」入口（路由 `/assignment`），用于课程作业的发布与提交。学生端、管理端前后端与提交判定链路均已完整实现。
...
- **未完成提醒**：导航栏「练习/作业」角标显示未完成必做题目数，截止或未完成时闪烁提醒
- **管理端（前端 + 后端）**：学生组管理（增删改查、添加/移除成员）、作业管理（创建/编辑/发布/延期/删除、完成统计）
...
| 2026-09-03 | **作业功能**：学生组管理、作业发布/提交链路、AC 制完成判定、未完成提醒（学生端 + 管理端前后端）；deploy.sh 支持多数据库迁移与 JudgeServer 热替换 |  lzdogbro   |
| 2026-09-03 | **修复**：Lombok 升级丢失 `@ConstructorProperties`，导致评测提交反序列化失败、全部显示 Submitted Failed |  lzdogbro   |
