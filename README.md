...
导航栏新增「作业」入口（路由 `/assignment`），用于课程作业的发布与提交。已完成学生端与管理端前后端、提交判定链路。

- **学生端**：作业列表、作业详情（标题、必做/选做、起止时间、进度条）、作业题目列表（每题 AC 状态）、作业内提交
- **管理端**：学生组管理（增删改查、添加/移除成员）、作业管理（新建草稿 / 一步发布 / 编辑 / 发布 / 延期 / 删除）
- **下发对象**：按学生组下发（可多选）+ 手动追加学生，并可从组内排除个别学生（被排除者不布置该作业）
- **发布后编辑**：已发布作业仍可修改标题 / 说明 / 必做状态 / 时间窗 / 题目集（不改变发布状态，不影响历史提交）
...
| 2026-09-09 | **作业功能增强**：已发布作业支持全量编辑（标题/说明/必做/时间窗/题目集）；下发作业时支持从学生组内排除个别学生 |  lzdogbro   |
